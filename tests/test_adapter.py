...
class TestBuildMeasurementAdapterGuards:
    ...
    def test_raises_lists_all_missing(self) -> None:
        settings = _fake_settings(meas_lm_model=None, meas_lm_entity_identification_prompt=None)
        with pytest.raises(RuntimeError, match="MEAS_LM_MODEL, MEAS_LM_ENTITY_IDENTIFICATION_PROMPT"):
            build_measurement_adapter(settings)
