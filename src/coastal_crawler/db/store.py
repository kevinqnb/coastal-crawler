...
# ---------------------------------------------------------------------------
# Extractions (site support — vote flow only; paper/extraction/entity reads
# for the results website live in db/warehouse_reader.py against the DuckDB
# warehouse, not here. See notes/coastal-crawler/builds/2026-08-12-warehouse-site-01.md.)
# ---------------------------------------------------------------------------

def get_extraction(session: Session, extraction_id: int) -> Extraction | None:
    """Return one extraction with its parent paper, for the vote route's
    existence check and redirect (`paper_id`/`page_number`)."""
    stmt = (
        select(Extraction)
...
def get_judgements(session: Session, extraction_ids: list[int]) -> dict[int, str | None]:
    """Return `{extraction_id: judgement}` for the given ids.

    `judgement` is recomputed on every vote (see `record_vote`), so it can't
    be baked into the periodically-rebuilt DuckDB warehouse — this is the
    one field the site still live-joins against Postgres, keyed by the
    warehouse's `extractions_fact.source_extraction_id` (== `Extraction.id`
    here). Ids with no matching row are simply absent from the result.
    """
    if not extraction_ids:
        return {}
    stmt = select(Extraction.id, Extraction.judgement).where(Extraction.id.in_(extraction_ids))
