...
def run_filter(batch_size: int = 50) -> tuple[int, int, int]:
    ...
    Each paper is checked in order:
      ...
      2. LLM judge → relevant or irrelevant

    PDF accessibility is intentionally NOT checked here. Title/abstract are
    already in the DB from discovery, so relevance can be judged with zero
    network calls. Whether the PDF is actually downloadable is deferred to
    extraction (worker.py) — the only stage that truly needs the full file —
    so the (cheap, self-hosted) LLM filter is never bottlenecked by an
    external PDF host's rate limit (e.g. Wiley's TDM API). Papers whose PDF
    turns out to be unreachable surface as status='failed' during
    extraction with a descriptive error, not as a separate filter-time
    status.

    Status transitions:
      discovered  → filtering     (claimed)
      filtering   → relevant      (LLM says yes)
      filtering   → irrelevant    (no abstract, or LLM says no)
      filtering   → discovered    (API error — reset for retry next run)

    Returns:
        (relevant, irrelevant, errors) where errors are papers reset to
        'discovered' due to LLM API failures.
    """
